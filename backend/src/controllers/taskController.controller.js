...
const createTask = asyncHandler(async (req, res, next) => {
  ...
  const taskList = await Taskslist.findOne({taskcreatedDateId:taskListid, owner:userId})
  ...
  if(taskList){
    ...
    await taskList.save();
    logger.info(`new task added to the task list by user ${userId} with id ${taskList._id}`)
  }
  if(!taskList){
    ...
    logger.info(`new task list created by user ${userId} with id ${newtaskList._id}`)
...
const deleteTask = asyncHandler(async (req, res, next) => {
  ...
  return res.status(200).json(new apiResponse(201, {}, "task deleted"));
});
